...
export function ReferenceSolutionTab(props: ReferenceSolutionTabProps) {
  const { solutions, testCases, onChangeSolutions, runEndpoint, onRunResultsChange } = props;
  ...
  useEffect(() => {
    if (onRunResultsChange) {
      const summary: Record<string, { passedCount: number; totalCount: number }> = {};
      for (const [id, result] of Object.entries(runResults)) {
        summary[id] = { passedCount: result.passedCount, totalCount: result.totalCount };
      }
      onRunResultsChange(summary);
    }
  }, [runResults, onRunResultsChange]);
  ...
  const runSolution = async (solution: ReferenceSolution) => {
    ...
    try {
      const endpoint = runEndpoint || `${getBackendURL()}/admin/problems/run-reference-solution`;
