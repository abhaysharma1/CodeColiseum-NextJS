...
interface ProblemTabsProps {
  ...
  onChangePerformanceConstraints?: (constraints: PerformanceConstraints) => void;
  onChangePerformanceTestCases?: (testCases: PerformanceTestCase[]) => void;
  runEndpoint?: string;
  onRunResultsChange?: (results: Record<string, { passedCount: number; totalCount: number }>) => void;
}
...
export function ProblemTabs(props: ProblemTabsProps) {
  const {
    ...
    onChangeSolutions,
    onChangePerformanceConstraints,
    onChangePerformanceTestCases,
    runEndpoint,
    onRunResultsChange,
  } = props;
    ...
    <div className="flex flex-col w-full h-full">
      ...
      >
        <div className="flex w-full items-center justify-center sm:justify-start overflow-x-auto pb-4 hide-scrollbar">
          <TabsList className="h-12 w-auto bg-muted/50 p-1 rounded-full grid grid-cols-5 gap-1">
            <TabsTrigger
        ...
        <div className="flex-1 mt-2">
          ...
          >
            <ReferenceSolutionTab
              ...
              onRunResultsChange={onRunResultsChange}
            />
          </TabsContent>
          ...
          >
            <PerformanceTab
              constraints={state.performanceConstraints}
              testCases={state.performanceTestCases}
              onChangeConstraints={onChangePerformanceConstraints}
              onChangeTestCases={onChangePerformanceTestCases}
            />
          </TabsContent>
        </div>
      </Tabs>
